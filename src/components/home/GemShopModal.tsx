...
import { X, Crown, Sparkles, Zap, Star, Check } from "lucide-react";
...
interface ShopItem {
  ...
  category: "avatar" | "powerup" | "vip" | "coins";
  value?: number; // For coin packs, this is how many coins you get
  gradient: string;
  popular?: boolean;
}
...
const SHOP_ITEMS: ShopItem[] = [
  ...
  // Exclusive Avatars
  {
    id: "avatar_galaxy",
    name: "Galaxy ავატარი",
    description: "ექსკლუზიური კოსმოსური ჩარჩო",
    price: 20,
...
export function GemShopModal({ isOpen, onClose }: GemShopModalProps) {
  ...
  const handlePurchase = async (item: ShopItem) => {
    ...
    try {
      ...
      // For other items, you'd implement their specific logic here
      // (e.g., add power-ups, activate VIP, unlock avatar)
                        ...
                        >
                          ...
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            <span className={`font-bold ${canAfford ? "text-white" : "text-red-300"}`}>
                              {isPurchased ? "შეძენილია" : item.price}
                            </span>
                          </div>
                        </motion.button>
                      </motion.div>
