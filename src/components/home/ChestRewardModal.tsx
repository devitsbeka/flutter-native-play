...
import { getChestCoins, getChestGems } from "@/config/rewardConfig";
...
/**
 * The chest pays a fixed amount — `getChestCoins` — plus a gem at weekends.
 * It used to draw a random number of coins and publish its odds; a set
 * amount has no odds to publish, and nothing in the app is left to chance.
 */
...
export function ChestRewardModal({ isOpen, onClose, onClaim }: ChestRewardModalProps) {
  ...
  // Lay out the (fixed) rewards when the modal opens and the chest is claimable
  useEffect(() => {
    if (isOpen && canClaimChest) {
      const coins = getChestCoins();
