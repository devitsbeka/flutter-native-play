    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[400px] p-0 gap-0 bg-background border-none rounded-3xl overflow-hidden">
        ...
        <div className="px-6 pt-8 pb-6">
          ...
          {/* Sign in with Apple first, on every platform — guideline 4.8
          ...
          <button
            ...
            disabled={loading}
            className="w-full h-12 rounded-2xl bg-black text-white flex items-center justify-center gap-3 font-medium text-base hover:bg-black/90 transition-colors disabled:opacity-50"
          >
          ...
          <GoogleSignInButton onClick={handleGoogleSignIn} disabled={loading} className="w-full mt-3" />
        </div>
      </DialogContent>
    </Dialog>
  );
