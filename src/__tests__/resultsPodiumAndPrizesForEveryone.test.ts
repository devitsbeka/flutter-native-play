/**
 * The results screen: a podium, and the prizes reported to every device.
 *
 * The owner saw one results screen where the winner, on their own phone,
 * saw nothing — "winner got nothing". The coins HAD moved: settle_room_round
 * settles once, on whichever device asks first, and answered every later
 * device `already_settled` with no deltas, so the winner was simply never
 * told. The function reads the ledger back on that answer, and the client
 * folds it to one prize line per seat. Since 20261108100000_no_wagering a
 * line is only ever a prize: nobody pays in, and nobody loses coins.
 *
...
describe("every device is told what moved", () => {
  ...
  it("and the no-wagering settlement still does, from the same reader", () => {
    expect(noWager).toMatch(/'deltas', public\.room_round_deltas\(p_game_id\), 'reason', 'already_settled'\);/);
  });

  it("the hook folds the deltas to a prize line per seat, and the screen keeps them", () => {
    expect(hook).toMatch(/function foldLines\(deltas: RoomRoundResponse\["deltas"\]\): Record<string, RoomPrizeLine>/);
    expect(hook).toMatch(/lines,\s*\n\s*unsettled: false,/);
    expect(results).toMatch(/setPrizeLines\(settlement\.lines\);/);
  });

  it("is proved against Postgres, and CI runs that file", () => {
    expect(sqlTest).toMatch(/a second call is a no-op/);
    expect(sqlTest).toMatch(/but still reports the prize/);
    expect(ci).toMatch(/-f supabase\/tests\/26-no-wagering\.sql/);
  });
});
...
describe("the podium", () => {
  ...
  it("a medal under each face, and the prize under the medal", () => {
    ...
    const coins = step.indexOf("<PrizeLine prize={prizeFor(p)} />");
  ...
  it("names no amount of its own — a seat's line comes from the server", () => {
    const fn = results.slice(results.indexOf("const prizeFor = "), results.indexOf("const hasUpdatedStats"));
    expect(fn).toMatch(/const line = prizeLines\[p\.user_id\];\s*\n\s*if \(line\) return line\.prize;/);
  });
