...
describe("what a settled public room will not let the host do", () => {
  ...
  it("and there is no other way in — every room write is behind one of these", () => {
    ...
    // Ten since publishDraft's own update was broken across lines to take
    // a `.select("id")` — the same write, not a new one.
    expect(writes, "a new game_rooms write needs a lock decision").toBe(10);
  });
});
