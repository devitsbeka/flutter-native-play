// Builds src/features/words/levels.<lang>.generated.ts — one language's
// Words level bank.
//
//   node scripts/words-levels/generate.mjs en     (or es, fr, de, it, pt, ka)
//
...
const BLOCK_BY_LANG = {
  ...
  de: ["scheisse", "arsch", "ficken", "fotze", "hure", "kacke", "pisse", "titten", "sex", "nutte"],
  ka: ["ყლე", "ტრაკი", "ბოზი", "მუტელი", "ძუკნა", "ბოზო", "სექსი"],
  en: [],
};
for (const w of BLOCK_BY_LANG[lang] ?? []) BLOCK.add(w);

// A word is usable when it is in the list and not blocked.
...
const MAX = 600;
// Georgian words run long and its usable list is smaller: let the wheel go
// to eight letters there so enough boards exist.

const out = levels.slice(0, MAX);

// Georgian has one case in everyday writing; upper-casing it would turn the
// letters into Mtavruli, which nobody spells with.
const up = (w) => (lang === "ka" ? w : w.toUpperCase());
const body = out
  .map((l) => `  { letters: ${JSON.stringify(up(l.letters))}, words: ${JSON.stringify(l.words.map(up))}, bonus: ${JSON.stringify(l.bonus.map(up))} },`)
  .join("\n");
writeFileSync(
  resolve(here, `../../src/features/words/levels.${lang}.generated.ts`),
  `// Generated by scripts/words-levels/generate.mjs ${lang} — do not edit by hand.\n// ${out.length} levels from the ${words.length} most common 3–7 letter words (${lang}).\n\nimport type { RawLevel } from "./levels";\n\nexport const RAW_LEVELS: RawLevel[] = [\n${body}\n];\n`,
);
console.log(`${lang}: wrote ${out.length} levels (${levels.length} candidates)`);
