CREATE OR REPLACE FUNCTION public.get_questions_sorted_by_length(p_sort_mode text DEFAULT 'longest_question'::text, p_in_production boolean DEFAULT false, p_category_id uuid DEFAULT NULL::uuid, p_language text DEFAULT NULL::text, p_question_type text DEFAULT NULL::text, p_difficulty text DEFAULT NULL::text, p_has_icon text DEFAULT NULL::text, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, category_id uuid, question_text text, correct_answer text, incorrect_answers jsonb, difficulty text, level_number integer, is_active boolean, in_production boolean, icon_slug text, image_url text, video_url text, audio_url text, created_at timestamp with time zone, updated_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
